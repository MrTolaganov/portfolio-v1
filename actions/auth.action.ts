...
export async function createUser(fullName: string, email: string): Promise<IUser> {
  try {
    await connectDatabase()
...
export async function signIn(email: string, password: string): Promise<IResponse> {
  try {
    await connectDatabase()
    ...
    const correctPassword = await bcryptjs.compare(password, existedUser.password)
...
export async function updatePassword(email: string, newPassword: string): Promise<IResponse> {
  try {
    await connectDatabase()
