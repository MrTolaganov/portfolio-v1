      ...
      <div className="h-screen flex flex-col items-center justify-center px-4 sm:px-8 md:px-16 lg:px-32">
        ...
        >
          <CarouselContent className={"h-[500px]"}>
            {techs.map((tech) => (
              <CarouselItem key={tech.image} className={`basis-1/3`}>
                <div className="mx-auto">
                  <Card className={"bg-background rounded-0 border-0"}>
                    <CardContent className="bg-background w-full flex aspect-square items-center justify-center relative border-0">
                      <Image
                        src={tech.image}
                        alt={tech.label}
                        fill
